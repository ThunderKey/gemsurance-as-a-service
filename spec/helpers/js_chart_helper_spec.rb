...
RSpec.describe ApplicationHelper, type: :helper do
  ...
  describe '#generate_colors' do
    ...
    it('10 colors') {
      expect(helper.generate_colors(10)).to eq [
        '#FF6666', '#FFC266', '#E0FF66', '#85FF66', '#66FFA3',
        '#66FFFF', '#66A3FF', '#8566FF', '#E066FF', '#FF66C2'
      ]
    }
    it('3 light colors') do
      expect(helper.generate_colors(3, lightness: 0.80)).to eq ['#FF9999', '#99FF99', '#9999FF']
    end
    it('3 dark colors') do
      expect(helper.generate_colors(3, lightness: 0.25)).to eq ['#800000', '#008000', '#000080']
    end
    it('3 colors with low saturation') do
      expect(helper.generate_colors(3, saturation: 0.5)).to eq ['#D98C8C', '#8CD98C', '#8C8CD9']
    end
  end
end
