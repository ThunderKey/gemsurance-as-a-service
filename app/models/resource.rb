...
class Resource < ApplicationRecord
  ...
  enum fetch_status: Hash[%i(pending successful failed).map {|k| [k, k.to_s] }]
  ...
  validates :path, presence: true, format: {
    with: ApplicationHelper.absolute_path_regex,
    message: :relative_path,
  }
  validates :resource_type, presence: true
  validates :build_image_url, format: {
    with: Rails.application.config.url_regex,
    message: :invalid_url,
    allow_blank: true,
  }
  validates :build_url, format: {
    with: Rails.application.config.url_regex,
    message: :invalid_url,
    allow_blank: true,
  }
  ...
  validate do
    gemsurance_service.errors.each {|k, m| errors.add k, m } if resource_type
  end
  ...
  def self.resource_type_attributes_for_select
    resource_types.keys.map do |resource_type|
      key = "activerecord.attributes.#{model_name.i18n_key}.resource_types.#{resource_type}"
      [I18n.t(key), resource_type]
    end
  end
  ...
  def gem_status
    if vulnerabilities_count.positive?
      :vulnerable
    else
      :current
    end
